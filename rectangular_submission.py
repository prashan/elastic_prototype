...
def extract(result,points,dimensions):
    ...
    return k_list,n_list,ajb_list,beta_list,c_list,v_list,omega,centers,sum_omega,d_list,g_list
...
def extract_and_output_solution(points_fn,report_file,points,N,dimensions,my_prob,B,lambda_o,pdf_pages,time_to_solve,parameter_str,names_dict,header,iteration,alt_type,feature_dict,class_dict):
    ...
    print 'start extract'
    k_list,n_list,ajb_list,beta_list,c_list,v_list,omega,centers,sum_omega,d_list,g_list=extract(result,points,dimensions)   
    print 'end extract'    
...
def setup_objective_and_contraints(prob,B,lambda_o,points,N,dimensions,alt_type,omega,v_list,alpha_prev,beta_prev,feature_dict,iteration):
    ...
    const1=time.time()
    add_time,time_in_loop=generate_constraints(prob,points,N,dimensions,B,name_index_dict,alt_type,omega,v_list,alpha_prev,beta_prev,feature_dict,num_classes) #set constraints                                   
    const2=time.time()
...
def run_MIP(points_fn,report_file,points,N,dimensions,B,lambda_o,pdf_pages,names_dict,header,test_points,data_set,fold):
    ...
    for alt_run in xrange(alt_runs): #change back to 2*alt_runs
        error_flag=0
        if alt_run%2==0:
            alt_type='space'
        else:
        ...
        try:
            ...
            if warm_start_flag==True:                              
                ...
                sample_point_names,ws_sol_names=create_ws_lists(N,data_set,fold)
                warm_start(my_prob,points,N,B,sample_point_names,ws_sol_names,dimensions,raw_pop)
                b=time.time()
                ws_time=round(b-a,2)
            ...
            cb.incobjval, cb.bestobjval,cb.times = [], [], []
            
            start=time.time()
            #solve the optimization problem            
            my_prob.solve()
#            my_prob.MIP_starts.write('./rectangular.mst')
            end=time.time()
            ...
            if error_flag!=1:
                cumulative_objective,objective_anno=plot_objective(cb,pdf_pages,iteration,alt_type,cumulative_objective,objective_anno,my_prob.solution.get_objective_value())
        ...
        train_k_list,train_n_list=compute_train_performance(alpha,omega,v_list,points,test_points,centers,B,feature_dict,pdf_pages,class_dict)
...
def compute_train_performance(alpha,non_zero_omega,non_zero_features,train_points,test_points,centers,B,feature_dict,pdf_pages,class_dict) :
    ...
    for i,v in test_points.items():
        ...
        for (j,b) in alpha:          
            ...
            if i_label==j_label and (d_ib[1]<dis or abs(d_ib[1]-dis)<1e-15):
                print 'ksi same label : i_label,i,j,b,d,1-epsilon',i_label,i,j,b,d_ib[1],dis
                temp_ksi=0
            elif d_ib[1]<=1:
                print 'ksi different label : i_label,i,j,b,d,1-epsilon',i_label,i,j,b,d_ib[1],dis
                
                    
                
            if i_label!=j_label and (d_ib[1]<dis or abs(d_ib[1]-dis)<1e-15):
                print 'eta: i_label,i,j,b,d,1-epsilon',i_label,i,j,b,d_ib[1],dis
                temp_eta=1
        #update the count 
        if temp_ksi==1:
    ...
    draw_plot(centers,non_zero_omega,non_zero_features,feature_dict,B,appended_points,pdf_pages,class_dict,train_points,'Test')
        
    return train_k_list,train_n_list
               

def plot_objective(cb,pdf_pages,iteration,alt_type,cumulative_objective,objective_anno,final_obj):
     # plot obj value   
    if print_to_report==True:
        plt.ion()   
        fig=plt.figure()
    ...
    if cb.times:
        cb.times.append(cb.times[-1]+1)
    else:
        cb.times.append(1)
    size = len(cumulative_objective)
    print 'times match',len(cumulative_objective),len(cb.times)
    if print_to_report==True:
        plt.plot(range(size), cumulative_objective,'g') 
        ax.set_xticklabels(cb.times)
        
    objective_anno[size-1]=(alt_type+str(iteration),cumulative_objective[-1])
    
    if print_to_report==True:
        for k,val in objective_anno.items():
            ax.annotate(val[0], xy=(k,val[1]),  xycoords='data',textcoords='data', ha='center', va='bottom',size=7,rotation=90,arrowprops=dict(arrowstyle='->', connectionstyle='arc3,rad=0.5',color='red'))    
                      
        pdf_pages.savefig(fig)
        plt.ioff()
        plt.close()
    return cumulative_objective,objective_anno
...
def draw_plot(centers,omega,v_list,feature_dict,B,points,pdf_pages,class_dict,train_points,split):
    radaii_centers={}
    try :
...
#                print 'ball=',b,',class=',class_dict[int(points[int(centers[b][-2])][0])],': (dim, omega)\n',[(item[0],round(item[1],7)) for item in sorted(value,key=lambda x: x[0])]
                radii=[1/item[1] if item[1]!=0 else 1.0/w1_min for item in sorted(value,key=lambda x: x[0])]
                cube_centers=centers[b][:-2]
                j_b=centers[str(b)][-2:]
                radaii_centers[b]=(cube_centers,radii,j_b)
    ...
    if len(v_list)==3:
        if centers.keys():
            draw_cube(radaii_centers,v_list,feature_dict,B,points,pdf_pages,train_points,split) 
    elif len(v_list)<3:
        if centers.keys():
            draw_rectangle(radaii_centers,v_list,feature_dict,B,points,pdf_pages,train_points,split)     
...
def print_solution(my_prob,points,dimensions,sol,k_list,n_list,c_list,centers,sum_omega,lambda_o,pdf_pages,time_to_solve,parameter_str,names_dict,header,iteration,alt_type,report_file,feature_dict,d_list,g_list,class_dict):
    ...
    print 'nonzero c',c_list,'\n'
    print 'nonzero d',d_list,'\n'
#    print 'nonzero g',g_list,'\n''
    ''    
    ...
    draw_plot(centers,omega,v_list,feature_dict,B,points,pdf_pages,class_dict,{},'Train')
    ...
    for k,val in class_dict.items():
        ...
        for (j,b),i_list in incorrect.items()+no_miss_dict.items():
            ...
            print '         axis(from smallest): ',pp.pprint([feature_dict['v'+str(tup[0])]+': '+str(1/tup[1])+' ' if tup[1]>1e-8 else feature_dict['v'+str(tup[0])]+': '+str(1/w1_min)+' ' for tup in sorted(omega[b],key=lambda x: x[1],reverse=True)])
...
def draw_cube(radaii_centers,v_list,feature_dict,balls,points,pdf_pages,train_points,split):
    ...
    for b,value in radaii_centers.items():
        ...
        #plot the centers
        ax.scatter3D(c1,c2,c3,color=class_dict[class_type][0], marker='s')
        
        
    plt.title('Cube plot for '+str(split))
...
#            continue
        ...
        #plot the centers
        ax.scatter(c1,c2,color=class_dict[class_type][0], marker='s')
        
        
    plt.title('Rectangular plot '+str(split))
